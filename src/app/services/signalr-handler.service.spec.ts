...
describe('SignalrHandlerService', () => {
  ...
  beforeEach(() => {
    const apiServiceSpy = jasmine.createSpyObj('ApiService', ['markAsRead']);
    ...
    const signalrServiceSpy = jasmine.createSpyObj('SignalrService', ['init', 'removeHandlers']);
    const browserNotificationServiceSpy = jasmine.createSpyObj('BrowserNotificationService', ['showNotification']);
    ...
    TestBed.configureTestingModule({
      providers: [
        ...
        { provide: BrowserNotificationService, useValue: browserNotificationServiceSpy },
      ],
    });
    ...
    browserNotificationService = TestBed.inject(BrowserNotificationService) as jasmine.SpyObj<BrowserNotificationService>;
  });

  describe('initHandlers', () => {
    it('should initialize SignalR handlers', async () => {
      const handleMessageNotification = jasmine.createSpy('handleMessageNotification');
      const handleLinkPreviewNotification = jasmine.createSpy('handleLinkPreviewNotification');
      const handleImagePreviewNotification = jasmine.createSpy('handleImagePreviewNotification');

      await service.initHandlers(handleMessageNotification, handleLinkPreviewNotification, handleImagePreviewNotification);
  ...
  describe('handleMessageNotification', () => {
    ...
    beforeEach(() => {
      ...
      chats = [{ id: 'chatId', chatName: 'Chat 1', unreadCount: 0 }] as IChatDto[];
      isWindowActive = false;
    });

    it('should set last message info and add message if chatId matches selectedChatId', async () => {
      await service.handleMessageNotification(messageDto, selectedChatId, chats, isWindowActive);
    ...
    it('should mark message as read if window is active and chatId matches selectedChatId', async () => {
      ...
      expect(apiService.markAsRead).toHaveBeenCalledWith(messageDto.chatId, messageDto.id);
    });
  ...
  describe('handleLinkPreviewNotification', () => {
    it('should set link preview if chatId matches selectedChatId', () => {
      const linkPreviewDto: ILinkPreviewDto = { chatId: 'chatId', url: 'http://example.com' } as ILinkPreviewDto;
      const selectedChatId = 'chatId';

      service.handleLinkPreviewNotification(linkPreviewDto, selectedChatId);
  ...
  describe('handleImagePreviewNotification', () => {
    it('should set image preview if chatId matches selectedChatId', () => {
      const imagePreviewDto: IImagePreviewDto = { chatId: 'chatId', imageId: '1' } as IImagePreviewDto;
      const selectedChatId = 'chatId';

      service.handleImagePreviewNotification(imagePreviewDto, selectedChatId);
